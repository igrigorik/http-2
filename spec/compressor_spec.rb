...
RSpec.describe HTTP2::Header do
      ...
      context 'choosing shorter representation' do
        [['日本語', :plain],
         ...
         ['xq', :plain] # prefer plain if equal size
          ...
          it "returns #{choice} representation" do
  ...
  context 'header representation' do
    it 'handles indexed representation' do
    ...
    it 'raises when decoding indexed representation with index zero' do
      h = { name: 10, type: :indexed }
      wire = c.header(h)
  ...
  context 'shared compression context' do
    before { @cc = EncodingContext.new }

    it 'is initialized with empty headers' do
    ...
    context 'processing' do
      [
        ...
        context desc.to_s do
          it 'processes indexed header with literal value' do
          ...
          it 'processes literal header with literal value' do
            original_table = @cc.table.dup
      ...
      context 'size bounds' do
        it 'drops headers from end of table' do
      ...
      it 'clears table if entry exceeds table size' do
        cc = EncodingContext.new(table_size: 2048)
        cc.instance_eval do
          add_to_table(['test1', '1' * 1024])
          add_to_table(['test2', '2' * 500])
      ...
      it 'shrinks table if set smaller size' do
        cc = EncodingContext.new(table_size: 2048)
        cc.instance_eval do
          add_to_table(['test1', '1' * 1024])
          add_to_table(['test2', '2' * 500])
      ...
      it 'rejects table size update if exceed limit' do
      ...
      context "spec example #{ex[:title]}" do
        ex[:streams].size.times do |nth|
          context "request #{nth + 1}" do
            ...
            before do
              @dc = Decompressor.new(table_size: ex[:table_size])
              (0...nth).each do |i|
            ...
            if ex[:streams][nth][:has_bad_headers]
              it 'raises CompressionError' do
                bytes = [ex[:streams][nth][:wire].delete(" \n")].pack('H*')
                expect { @dc.decode(HTTP2::Buffer.new(bytes)) }.to raise_error ProtocolError
              end
            else

              it 'emits expected headers' do
                subject
                # partitioned compare
