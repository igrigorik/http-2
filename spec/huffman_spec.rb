...
RSpec.describe HTTP2::Header::Huffman do
  ...
  context 'encode' do
    before(:all) { @encoder = described_class.new }

    huffman_examples.each do |plain, encoded|
      it "encodes #{plain} into #{encoded}" do
  ...
  context 'decode' do
    ...
    huffman_examples.each do |plain, encoded|
      it "decodes #{encoded} into #{plain}" do
    ...
    [
      ...
      it "encodes then decode '#{string}' into the same" do
    ...
    it 'encode/decodes all_possible 2-byte sequences' do
      (2**16).times do |n|
    ...
    it 'raises when input is shorter than expected' do
    ...
    it 'raises when input is not padded by 1s' do
    ...
    it 'raises when exceedingly padded' do
      encoded = 'e7cf9bebe89b6fb16fa9b6ffff' # NOTE: the extra ff
      encoded = [encoded].pack('H*')
      expect { @encoder.decode(HTTP2::Buffer.new(encoded)) }.to raise_error(/EOS invalid/)
    end

    it 'raises when EOS is explicitly encoded' do
